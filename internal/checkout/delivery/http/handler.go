...
func (h *Handler) StartPayment(c *gin.Context) {
	...
	started, err := h.service.StartPayment(c.Request.Context(), checkoutDomain.StartPaymentRequest{
		...
		CustomerID:  request.CustomerID, CustomerPhone: request.CustomerPhone, DeliveryProvider: request.DeliveryProvider, ReturnURL: request.ReturnURL, CancelURL: request.CancelURL,
	})
	if err != nil {

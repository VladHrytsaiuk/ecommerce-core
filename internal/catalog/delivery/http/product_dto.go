// Package http exposes the Catalog storefront API. It only maps HTTP to the
// application port; persistence and catalog rules stay outside Gin.
//
// The request DTOs that used to live here belonged to an unregistered admin
// create route. Admin writes go through the audited catalog facade, which
// records the change and its audit entry in one transaction.
package http

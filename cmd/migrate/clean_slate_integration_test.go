...
func TestCleanSlateSchema(t *testing.T) {
	...
	if err := runMigrations(root, databaseURL, []string{"admin", "availability_notifications", "catalog", "comparison", "customers", "inventory", "reports", "returns", "reviews", "sync", "user_profiles", "wishlist"}, "up"); err != nil {
	...
	if err := runMigrations(root, databaseURL, []string{"admin", "availability_notifications", "catalog", "comparison", "customers", "inventory", "reports", "returns", "reviews", "sync", "user_profiles", "wishlist"}, "down"); err != nil {

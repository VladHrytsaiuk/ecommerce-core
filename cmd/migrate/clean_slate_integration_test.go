...
func TestCleanSlateSchema(t *testing.T) {
	...
	if err := runMigrations(root, databaseURL, []string{"admin", "catalog", "comparison", "customers", "inventory", "reports", "returns", "reviews", "sync", "user_profiles", "wishlist"}, "up"); err != nil {
	...
	if err := runMigrations(root, databaseURL, []string{"admin", "catalog", "comparison", "customers", "inventory", "reports", "returns", "reviews", "sync", "user_profiles", "wishlist"}, "down"); err != nil {
		t.Fatalf("rollback clean-slate schema: %v", err)
	}
	assertTablesAbsent(t, db, "locales", "products", "product_variants", "user_oauth_identities", "oauth_authorization_attempts", "user_profiles", "customer_profiles", "customer_addresses", "product_media", "wishlist_items", "comparison_lists", "comparison_items", "reviews", "product_review_ratings", "roles", "permissions", "report_processed_events", "report_daily_sales", "report_daily_product_sales", "report_daily_funnel", "return_requests", "return_items", "return_status_history", "return_restock_operations", "warehouses", "stock_items", "inventory_reservations", "sync_outbox", "sync_external_entity_state", "sync_cursors")
}
